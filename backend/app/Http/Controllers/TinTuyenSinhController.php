...
class TinTuyenSinhController extends Controller
{
        ...
        try {
            $query = DB::table('tin_tuyensinh')
                ...
                ->select(
                    ...
                    'tin_tuyensinh.ngay_cap_nhat',
                    'tin_tuyensinh.ma_nguon',
                    'tin_tuyensinh.hash_noidung',
            ...
            \Log::error('Error in TinTuyenSinhController@index: ' . $e->getMessage());
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $data = [];
            if ($request->filled('id_truong')) {
                $data['id_truong'] = $request->integer('id_truong');
            }
            $data['tieu_de'] = $request->string('tieu_de');
            if ($request->filled('tom_tat')) {
                $data['tom_tat'] = $request->string('tom_tat');
            }
            if ($request->filled('hinh_anh_dai_dien')) {
                $data['hinh_anh_dai_dien'] = $request->string('hinh_anh_dai_dien');
            }
            if ($request->filled('nguon_bai_viet')) {
                $data['nguon_bai_viet'] = $request->string('nguon_bai_viet');
            }
            $data['loai_tin'] = $request->string('loai_tin');
            if ($request->filled('trang_thai')) {
                $data['trang_thai'] = $request->string('trang_thai');
            }
            if ($request->filled('ma_nguon')) {
                $data['ma_nguon'] = $request->string('ma_nguon');
            }
            // Note: ngay_het_han column has been removed from database
            
            // If a file is uploaded, upload to Cloudinary and store secure_url
            if ($request->hasFile('file_hinh_anh')) {
                try {
                    ...
                    if (empty($data['hinh_anh_dai_dien'])) {
    ...
    /**
     * Show news detail for admin (includes all statuses)
     */
    ...
    /**
     * Show news detail for admin (includes all statuses including pending)
     */
